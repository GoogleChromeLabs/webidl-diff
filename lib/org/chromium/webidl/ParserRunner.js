...
foam.CLASS({
  ...
  methods: [
    ...
    function run(file) {
      ...
      var metadata = Array.isArray(file.id) ? file.id.join('/') : file.id;
      ...
      if (!Array.isArray(results.value) ||
        ...
        this.error(this.fmtErrorMsg(
            `Incomplete parse on file. Metadata: ${metadata}`));
      } else {
        var message = this.ASTCollection.create({asts: results.value});
        this.output(message);
      }
    }
  ],
