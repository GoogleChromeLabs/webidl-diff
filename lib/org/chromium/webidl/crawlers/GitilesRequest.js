...
foam.CLASS({
  ...
  methods: [
    {
      name: 'getURL',
      documentation: 'Get the "canonical" URL for this resource.',
    ...
    function onResponse(response) {
      if (response.status !== 200) {
        ...
        response.payload.then(function(payload) {
          self.logError(`Unexpected response code: ${response.status}
            Gitiles request: ${self.getMachineURL()}
