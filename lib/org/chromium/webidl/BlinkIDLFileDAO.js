...
foam.CLASS({
  ...
  constants: {
    ...
    codeSearchQueryURL: 'https://cs.chromium.org/codesearch/json/search_request:1?search_request=b&query=file%3Athird_party%2FWebKit%2FSource.*idl%24+-file%3A%2Ftesting%2F+-file%3A%2Fbindings%2Ftests%2F&max_num_results=2000&internal_options=b&internal_options=e&search_request=e',
  ...
  properties: [
    ...
    },
    {
      ...
      documentation: `Internal state: Is the DAO ready to accept operations?
        May throw a faom.dao.InternalException. To automatically catch retry
  ...
  methods: [
    ...
    function put(obj) {
      var notReady = this.checkNotReady_();
    ...
    function find(objOrId) {
      ...
      return this.delegate.find(objOrId);
    },
    ...
    function removeAll(skip, limit, order, predicate) {
      ...
      return this.delegate.removeAll(skip, limit, order, predicate);
    },
  ...
  listeners: [
    function onCommitSet() {
      var self = this;
      return self.HTTPRequest.create({
        url: self.CODE_SEARCH_QUERY_URL,
        responseType: 'json',
      }).send().then(function(response) {
        if (response.status !== 200) throw response;
        return response.payload;
