...
foam.CLASS({
  ...
  listeners: [
    function onFetched() {
      this.commit = this.childProcess_.execSync(
          `"${this.GIT_PATH}" rev-parse HEAD`, {cwd: this.localRepositoryPath})
              .toString().trim();
      var execStr = `"${this.FIND_PATH}" "${this.localRepositoryPath}" -type f -path "${this.localRepositoryPath}/${this.SPARSE_PATH}/*.idl"`;
      for (var i = 0; i < this.FIND_EXCLUDE_PATTERNS.length; i++) {
        execStr += ` -not -path "${this.localRepositoryPath}*/${this.FIND_EXCLUDE_PATTERNS[i]}"`;
      }
      this.childProcess_.exec(execStr, this.onPaths);
    },
    function onPaths(error, stdout, sterr) {
    ...
    function onFileReady(data, resolve, reject, error, contents) {
      if (error) {
        reject(error);
      } else {
