      ...
      code: function(defnMap, key) {
        ...
        for (var i = 1; i < defns.length; i++) {
          ...
          if (changes.length !== 0) {
            ...
            delete defnMap[key];
            throw new Error(`Diff: ${key} was defined multiple times.
    ...
    },
    {
      ...
      code: function arrayCmp_(left, right, id, opt_changes) {
        ...
        for (var i = 0; i < left.length; i++) {
          var matched = false; // Used to determine if inner loop broke early.
          for (var j = 0; j < right.length; j++) {
            ...
            if (changes.length === 0 || leftName && rightName) {
              // If they have same name and changes were found,
              // persist the changes.
              this.chunks_ = this.chunks_.concat(changes);
              matched = true;
              break;
            }

            // Unnamed objects were different at this point.
            if (opt_changes) opt_changes.concat(changes);
          }
