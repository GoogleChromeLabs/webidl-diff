  ...
  methods: [
    ...
    function diff(left, right) {
      ...
      keys.forEach(function(key) {
        var multiDefine = function(def) {
          var changes = [];
          var id = {
            name: key,
            path: '',
          };
          this.diff_(def, def, id, changes);
          foam.assert(changes.length === 0,
              `${key} was defined multiple times, and they were not all the same`);

          var sources = def.map(function(d) {
            return d.sources;
          });
          def[0].sources = [].concat.apply([], sources);
        }.bind(this);

        // If we have an array of items (e.g. Enums / Typedef),
        // they should all be the same... Otherwise, no idea how to handle.
        if (Array.isArray(left[key])) {
          multiDefine(left[key]);
          // If nothing threw, we can just take first occurrence.
          left[key] = left[key][0];
        }
        if (Array.isArray(right[key])) {
          multiDefine(right[key]);
          right[key] = right[key][0];
        }
    ...
    function diff_(left, right, id, opt_changes) {
      ...
      if (['boolean', 'number', 'string'].includes(leftType) || !left || !right) {
        if (left !== right)
      ...
      if (isLeftArray) {
        ...
        var rightCls = right.cls_.name || null;
        ...
        if (foam.util.compare(leftProps, rightProps) !== 0) {
          // We cannot perform a deep comparison of the objects. Diff ends here.
          this.addChunk_(id, this.DiffStatus.VALUES_DIFFER, left, right, opt_changes);
          return;
        } 

        // Use leftProps since it is same.
      ...
      for (var i = 0; i < left.length; i++) {
        ...
        for (var j = 0; j < right.length; j++) {
          ...
          if (leftName !== rightName) {
            // Different name likely means they are different definitions.
            continue;
