...
foam.CLASS({
  ...
  methods: [
    function run(args) {
      if (!args || !args.config) throw new Error("LocalGitRunner: Missing configuration file!");
      ...
      if (!config.repositoryURL || !config.localRepositoryPath || !config.idlFileContentsFactory)
        throw new Error("LocalGitRunner: Invalid configuration file");
    ...
    },
    {
      ...
      code: function(path, resolve, reject, error, contents) {
        if (error) {
          ...
          var urls = this.urlExtractor.extract(contents);
