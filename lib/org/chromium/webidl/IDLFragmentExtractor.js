...
foam.CLASS({
  ...
  constants: [
    {
      ...
      documentation: 'Tags found in HTML files that do not have closing tags.',
      value: [ 'dd', 'dt', 'dl' ],
    },
  ],
  ...
  methods: [
    {
      name: 'isExcluded',
      documentation: 'Determines if class array includes excluded tags.',
      code: function(cls) {
        ...
        for (var i = 0; i < results.length; i++) {
          ...
          if (!tagMatching) {
            ...
            if (item.type.name === OPEN) {
              if (this.SELF_CLOSING_TAGS.includes(item.nodeName)) {
                // We don't do anything... Just skip it
              } else if (this.isExcluded(classes)) {
