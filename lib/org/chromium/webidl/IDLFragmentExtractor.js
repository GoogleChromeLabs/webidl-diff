...
foam.CLASS({
  ...
  properties: [
    ...
    {
      name: 'idlFragments',
      documentation: 'Array of IDLFragments found in the parsed HTML file.',
    },
  ],
  ...
  methods: [
    function extract() {
      ...
      var ignoreNextPre = false;    // Used to ignore pre tag following IDL Index.
        ...
        if (!tagMatching) {
          ...
          if (isTag) {
            if (item.nodeName === 'pre') {
              // Finished matching pre tag at this point.
              tagMatching = true;
              tagStack.pop();
          ...
          var id = extractAttr(item, 'id');

          // TODO: Replace with more elegant solution.
          // Currently it ignores the pre body immediately following
          // a tag with id of 'idl-index'.
          if (id.includes('idl-index')) {
            // Entering idl-index paragraph.
            // Ignoring next IDL fragment as it is an index.
            ignoreNextPre = true;
          }
