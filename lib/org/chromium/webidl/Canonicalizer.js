...
foam.CLASS({
  ...
  documentation: `This component expects to receive AST nodes from different
    web platform rendering engines. Canonical versions of interface definitions
    will be created from AST nodes of the same engine.
  ...
  properties: [
    ...
    },
    {
      documentation: `A map of maps containing canonical IDL files. The
        ...
        files were extract. For example, sourceMap_['blink'] is a map that
  ...
  methods: [
    {
      ...
      code: function(source, ast, metadata) {
        ...
        var map = this.sourceMap_[source] = this.sourceMap_[source] || {};
        var def = ast.definition;

        // Ignore definition if Enum or Typedef.
        if (this.Enum.isInstance(def) || this.Typedef.isInstance(def)) {
          return;
        }
